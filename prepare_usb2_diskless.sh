...
mkdir -p /media/boot
mount "$BOOT" /media/boot
FINISH_SCRIPT="/media/boot/finish_usb2_diskless3.sh"
cp /media/usb/pre_finish_usb2_diskless3.sh "$FINISH_SCRIPT"
sed -n '1,/DEFAULT_DISK=none/{/DEFAULT_DISK=none/!p}' /usr/sbin/setup-alpine >> "$FINISH_SCRIPT"
cat /media/usb/post_finish_usb2_diskless3.sh >> "$FINISH_SCRIPT"
umount /media/boot
rmdir /media/boot
